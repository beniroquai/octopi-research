...
class AcquisitionStartResult:
    ...
    def __init__(self,
        ...
        if not exception is None:
            assert type in (None, "exception",
                            AcquisitionStartResultType.RaisedException)
...
class CameraPixelFormat:
    ...
    # instances of gx.GxPixelFormatEntry are represented as an int
    gx_pixel_format: Union[gx.GxPixelFormatEntry, int]
...
class HOME_OR_ZERO:  # enum
    # motor moves along the negative direction (MCU coordinates)
    HOME_NEGATIVE: int = 1
    # motor moves along the negative direction (MCU coordinates)
    HOME_POSITIVE: int = 0
    ZERO: int = 2
...
class WellplateFormatPhysical:
    ...
    def well_index_to_name(self, row: int, column: int, check_valid: bool = True) -> str:
        if check_valid:
            assert row >= (0+self.number_of_skip) and row <= (self.rows -
                                                              self.number_of_skip-1), f"{row=} {column=}"
            assert column >= (0+self.number_of_skip) and column <= (
                self.columns-self.number_of_skip-1), f"{row=} {column=}"
    ...
    def well_name_to_index(self, name: str, check_valid: bool = True) -> Tuple[int, int]:
        # first character must be letter denoting the row, second and third character must be integer representing the column index (the latter starting at 1, not 0, and single digit numbers must be preceded by a 0)
        assert len(name) == 3, name

        row = ord(name[0])-ord('A')
        # because column numbering starts at 1, but indices start at 0
        column = int(name[1:])-1

        if check_valid:
            assert row >= (
                0+self.number_of_skip) and row <= (self.rows-self.number_of_skip-1), name
            assert column >= (
                0+self.number_of_skip) and column <= (self.columns-self.number_of_skip-1), name
    ...
    def is_well_reachable(self, row: int, column: int) -> bool:
        ...
        well_reachable = (row >= row_lower_bound and row <= row_upper_bound) and (
            column >= column_lower_bound and column <= column_upper_bound)

        if self.corners_forbidden:
            is_in_top_left_corner = (row == row_lower_bound) and (
                column == column_lower_bound)
            is_in_bottom_left_corner = (row == row_upper_bound) and (
                column == column_lower_bound)
            is_in_top_right_corner = (row == row_lower_bound) and (
                column == column_upper_bound)
            is_in_bottom_right_corner = (row == row_upper_bound) and (
                column == column_upper_bound)

            well_reachable = well_reachable and not (
                is_in_top_left_corner or is_in_bottom_left_corner or is_in_top_right_corner or is_in_bottom_right_corner)
    ...
    def pos_mm_to_well_index(self, x_mm: float, y_mm: float, return_nearest_valid_well_instead_of_none_if_outside: bool = False) -> Optional[Tuple[int, int]]:
        ...
        if x_mm >= origin_x and y_mm >= origin_y:
            ...
            if x_well_edge_distance <= self.well_size_mm and y_well_edge_distance <= self.well_size_mm:
                ...
                # print(f"inside well {self.well_index_to_name(row=y_wells,column=x_wells,check_valid=False)} with remaining {x_well_edge_distance=} {y_well_edge_distance=}")
                pass
                return row, column
    ...
    def limit_unsafe(self, calibrated: bool = False) -> "SoftwareStagePositionLimits":
        ...
        x_start_mm = x_start_mm + \
            (physical_wellplate_format.number_of_skip-1) * \
            physical_wellplate_format.well_spacing_mm
        ...
        x_end_mm = x_start_mm + (physical_wellplate_format.columns - 1 - physical_wellplate_format.number_of_skip*2) * \
            physical_wellplate_format.well_spacing_mm + \
            physical_wellplate_format.well_size_mm
        y_end_mm = y_start_mm + (physical_wellplate_format.rows - 1 - physical_wellplate_format.number_of_skip*2) * \
            physical_wellplate_format.well_spacing_mm + \
            physical_wellplate_format.well_size_mm
    ...
    def fov_exceeds_well_boundary(self, well_row: int, well_column: int, x_mm: float, y_mm: float) -> bool:
        ...
        well_center_x_mm, well_center_y_mm = self.well_index_to_mm(
...
assert WELLPLATE_FORMATS["Generic 384"].well_name_to_index(
    "A01", check_valid=False) == (0, 0)
assert WELLPLATE_FORMATS["Generic 384"].well_name_to_index(
    "B02", check_valid=False) == (1, 1)
assert WELLPLATE_FORMATS["Generic 384"].well_index_to_name(
    row=0, column=0, check_valid=False) == "A01"
assert WELLPLATE_FORMATS["Generic 384"].well_index_to_name(
    row=1, column=1, check_valid=False) == "B02"
...
class MachineConfiguration:
    ...
    FLIP_IMAGE: ClosedSet[Optional[str]](
        None, 'Vertical', 'Horizontal', 'Both') = None
    ...
    # 0.012*25.4 was written here at some point, not sure why. the motor makes _the_ weird noise during homing when set to the latter term, instead of 0.3
    ...
    # default sleep time between checks for microcontroller busy-ness
    SLEEP_TIME_S: float = 0.005
    # in busy loop where microcontroller packet presence is checked, wait this long between checks if a check has been unsuccessfull
    MICROCONTROLLER_PACKET_RETRY_DELAY: float = 1.0e-4
    ...
    TRACKERS: ClassVar[List[str]] = ['csrt', 'kcf',
                                     'mil', 'tld', 'medianflow', 'mosse', 'daSiamRPN']
    DEFAULT_TRACKER: ClosedSet[str](
        'csrt', 'kcf', 'mil', 'tld', 'medianflow', 'mosse', 'daSiamRPN') = 'csrt'

    AF: AutofocusConfig = AutofocusConfig()

    SOFTWARE_POS_LIMIT: SoftwareStagePositionLimits = WELLPLATE_FORMATS[MutableMachineConfiguration.WELLPLATE_FORMAT].limit_safe(
        calibrated=False)

    ENABLE_STROBE_OUTPUT: bool = False

    Z_STACKING_CONFIG: ClosedSet[str](
        'FROM CENTER', 'FROM BOTTOM', 'FROM TOP') = 'FROM CENTER'
    ...
    # upper left of B2 (corner opposite from clamp)
    X_ORIGIN_384_WELLPLATE_PIXEL: int = 177
    # upper left of B2 (corner opposite from clamp)
    Y_ORIGIN_384_WELLPLATE_PIXEL: int = 141
    ...
    # display laser af image every time when displacement is measured (even in multi point acquisition mode)
    ...
    # when moving to target, if absolute measured displacement after movement is larger than this value, repeat move to target (repeat max once) - note that the usual um/pixel value is 0.4
    LASER_AUTOFOCUS_TARGET_MOVE_THRESHOLD_UM: float = 0.3
    # when moving to target, no matter the measured displacement, move not further away from the current position than this value
    LASER_AUTOFOCUS_MOVEMENT_BOUNDARY_LOWER: float = -170.0
    # when moving to target, no matter the measured displacement, move not further away from the current position than this value
    LASER_AUTOFOCUS_MOVEMENT_BOUNDARY_UPPER: float = 170.0
    # when moving, move again max this many times to reach displacement target
    LASER_AUTOFOCUS_MOVEMENT_MAX_REPEATS: int = 3
    ...
    MUTABLE_STATE: MutableMachineConfiguration = {
        ...
        "WELLPLATE_FORMAT": "Generic 384",
        "DEFAULT_OBJECTIVE": "10x (Mitutoyo)"
    }
    DISPLAY: MachineDisplayConfiguration = {
        "DEFAULT_SAVING_PATH": "/home/pharmbio/Downloads"
    }
    ...
    def from_file(filename: str) -> "MachineConfiguration":
        ...
        if 'MUTABLE_STATE' in kwargs:
            mutable_state = MutableMachineConfiguration.from_json(
                kwargs['MUTABLE_STATE'])
...
MACHINE_CONFIG.SOFTWARE_POS_LIMIT = WELLPLATE_FORMATS[MACHINE_CONFIG.MUTABLE_STATE['WELLPLATE_FORMAT']].limit_unsafe(
    calibrated=True)
